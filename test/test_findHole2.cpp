...
int main(int argc, char** argv) {
    ...
    while (run) {
        ...
        //findContours
        cv::threshold(xyz[2], cropImage_edge, 254, 255, cv::THRESH_BINARY);
        ...
        for(size_t i = 0 ; i < cropImage_boundRect.size(); i++)
        {
            ...
            cropcropImage_up = Mat(cropImage, rect_up);
            ...
            cropcropImage_down = Mat(cropImage, rect_down);
            ...
            cv::threshold(xyz_up[2], xyz_up[2], 254, 255, cv::THRESH_BINARY);
            cv::threshold(xyz_down[2], xyz_down[2], 254, 255, cv::THRESH_BINARY);
